...
class MultipleAdversarialDebiasing(Transformer):
    ...
    def _adversary_model_equal_opportunity(self, pred_logits, true_labels):
        """Compute the adversary predictions for the protected attribute.
        """
        with tf.variable_scope("adversary_model"):
            # restrict to prediction=outcome
            pred_logits_eo = pred_logits[pred_logits==true_labels]
            true_labels_eo = true_labels[pred_logits==true_labels]
            
            c = tf.get_variable('c', initializer=tf.constant(1.0))
            s = tf.sigmoid((1 + tf.abs(c)) * pred_logits_eo)

            W2 = tf.get_variable('W2', [3, 1],
                                 initializer=tf.initializers.glorot_uniform(seed=self.seed4))
            b2 = tf.Variable(tf.zeros(shape=[1]), name='b2')

            pred_protected_attribute_logit = tf.matmul(tf.concat([s, s * true_labels_eo, s * (1.0 - true_labels_eo)], axis=1), W2) + b2
            pred_protected_attribute_label = tf.sigmoid(pred_protected_attribute_logit)
    ...
    def fit(self, dataset):
        ...
        with tf.variable_scope(self.scope_name):
            ...
            if self.debias:
                ...
                pred_protected_attributes_labels_eo, pred_protected_attributes_logits_eo = self._adversary_model_equal_opportunity(pred_logits, self.true_labels_ph)
                ...
                #weight this loss function
                pred_protected_attributes_loss = 0.5*pred_protected_attributes_loss_parity + 0.5*pred_protected_attributes_loss_eo 
            ...
            if self.debias:
                pa_opt = tf.train.AdamOptimizer(learning_rate)
                eo_opt = tf.train.AdamOptimizer(learning_rate)
            ...
            if self.debias:
                ...
                parity_adversary_vars = [var for var in tf.trainable_variables(scope=self.scope_name) if 'adversary_model' in var.name]
                eo_adversary_vars = [var for var in tf.trainable_variables(scope=self.scope_name) if 'adversary_model' in var.name]
                ...
                #maybe use:
                # pred_protected_attributes_loss_parity 
                # pred_protected_attributes_loss_eo 
                # instead of pred_protected_attributes_loss
                parity_adversary_grads = {var: grad for (grad, var) in pa_opt.compute_gradients(pred_protected_attributes_loss,
                                                                                      var_list=classifier_vars)}
                eo_adversary_grads = {var: grad for (grad, var) in eo_opt.compute_gradients(pred_protected_attributes_loss,
                                                                                      var_list=classifier_vars)}
            ...
            for (grad,var) in classifier_opt.compute_gradients(pred_labels_loss, var_list=classifier_vars):
                ...
                if self.debias:
                    ...
                    #NEW
                    unit_adversary_grad = normalize(parity_adversary_grads[var])
                    pa_grad -= tf.reduce_sum(grad * unit_adversary_grad) * unit_adversary_grad
                    pa_grad -= self.adversary_loss_weight * parity_adversary_grads[var]
                    
                    unit_adversary_grad = normalize(eo_adversary_grads[var])
                    eo_grad -= tf.reduce_sum(grad * unit_adversary_grad) * unit_adversary_grad
                    eo_grad -= self.adversary_loss_weight * eo_adversary_grads[var]
            ...
            if self.debias:
                ...
                with tf.control_dependencies([classifier_minimizer]):
                    # pred_protected_attributes_loss_parity --> maybe this?
                    pa_adversary_minimizer = pa_opt.minimize(pred_protected_attributes_loss, var_list=parity_adversary_vars)#, global_step=global_step)
                
                with tf.control_dependencies([classifier_minimizer]):
                    # pred_protected_attributes_loss_eo -->maybe this?
                    eo_adversary_minimizer = eo_opt.minimize(pred_protected_attributes_loss, var_list=eo_adversary_vars)#, global_step=global_step)
            ...
            for epoch in range(self.num_epochs):
                ...
                for i in range(num_train_samples//self.batch_size):
                    ...
                    if self.debias:
                        ...
                        #PARITY
                        _, _, pred_labels_loss_value, pred_protected_attributes_loss_vale = self.sess.run([classifier_minimizer,
                                       pa_adversary_minimizer,
                                       pred_labels_loss,
                                       pred_protected_attributes_loss], feed_dict=batch_feed_dict)
                        
                        #EQUAL OPP
                        _, _, pred_labels_loss_value, pred_protected_attributes_loss_vale = self.sess.run([classifier_minimizer,
                                       eo_adversary_minimizer,
                                       pred_labels_loss,
                                       pred_protected_attributes_loss], feed_dict=batch_feed_dict)
